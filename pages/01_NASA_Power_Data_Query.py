...
st.warning("This tool is a GIS-formatting utility for the [NASA POWER API](https://power.larc.nasa.gov/docs/tutorials/). Please read through the specifics as this page uses pre-configured formats (UTC, re) that can be individualized in a script. For real-time critical safety decisions, always cross-reference with NASA POWER Official.")
...
row1_col1, row1_col2 = st.columns(spec=2, border=True, gap="large")
#### Column 1: Date
with row1_col1:
...
if st.toggle("Help with Parameters"):
    ...
    """
    st.image("Screenshot 2026-01-26 at 17.00.26.png")
    """
...
st.divider()
#### Row 3
row3_col1, row3_col2, row3_col3 = st.columns(spec=3, border=True, gap="large")

#### Column 1: Site Elevation
with row3_col1:
    site_elevation = st.number_input("Site Elevation (Meters):", format="%.3f")
    if debug: print(site_elevation)

#### Column 2: Wind Elevation and Surface
with row3_col2:
